...
public class ThinkerAbility : PlayableCharacterComponent
{
    ...
    private SoundObject cheatedSnd = Resources.FindObjectsOfTypeAll<SoundObject>().ToList().Find(x => x.name == "CashBell"), blindSnd = Resources.FindObjectsOfTypeAll<SoundObject>().ToList().Find(x => x.name == "LAt_BlindStart"), blindLoop = Resources.FindObjectsOfTypeAll<SoundObject>().ToList().Find(x => x.name == "LAt_BlindLoop");
    ...
    public override void Initialize()
    {
        ...
        if (CoreGameManager.Instance.GetPoints(pm.playerNumber) < 50)
            CoreGameManager.Instance.AddPoints(Mathf.Abs(CoreGameManager.Instance.GetPoints(pm.playerNumber)) + Mathf.Min(50, 50 - Mathf.Abs(CoreGameManager.Instance.GetPoints(pm.playerNumber))), 0, false, false, false);
    }
    ...
    private static FieldInfo
        ...
        ___hasActivity = AccessTools.DeclaredField(typeof(RoomController), "hasActivity");
    private LayerMask layerMask = LayerMask.GetMask("Default", "Block Raycast", "Windows");
    ...
    private void Update()
    {
        if (pm.ec.Active && pm.plm.Entity.CurrentRoom != null)
        {
            ...
            hitCount = Physics.RaycastNonAlloc(ray, hits, Mathf.Min(pm.pc.reach*5, pm.ec.MaxRaycast), layerMask, QueryTriggerInteraction.Collide);
                ...
                if (timeLooking > 1f && _hitTransforms.Exists(x => x.gameObject.GetComponentInParent<Activity>() != null))
                {
                    ...
                    if (machine is MathMachine)
                    {
                        ...
                        if (answer.text == "?")
                        {
                            ...
                            mathMachine.gameObject.GetComponent<AudioManager>().PlaySingle(cheatedSnd);
                        }
                        else
                            timeLooking = 0f;
                    }
                    else if (machine is BalloonBuster)
                    {
                        ...
                        if (answer.text == "")
                        {
                            ...
                            answer.color = Color.green;
                            busterMachine.gameObject.GetComponent<AudioManager>().PlaySingle(cheatedSnd);
                        }
                        else
                            timeLooking = 0f;
                    }
    ...
    private IEnumerator ThinkerDrain()
    {
        ...
        while (pm.ec.Active)
        {
            ...
            while (CoreGameManager.Instance.GetPoints(pm.playerNumber) > 0)
            {
                ...
                if (!pm.ec.Active)
                    yield break;
                if (CoreGameManager.Instance.GetPoints(pm.playerNumber) > 0) CoreGameManager.Instance.AddPoints(CoreGameManager.Instance.GetPoints(pm.playerNumber) < 10 ? -CoreGameManager.Instance.GetPoints(pm.playerNumber) : -10, 0, true, CoreGameManager.Instance.GetPointsThisLevel(pm.playerNumber) >= 10, false);
                yield return null;
            }
            if (CoreGameManager.Instance.GetPoints(pm.playerNumber) <= 0 && !blinded)
            {
                blinded = true;
                pm.ec.AddFog(ThinkerBlind);
                musicMan.QueueAudio(blindSnd, true);
                musicMan.QueueAudio(blindLoop);
