...
public class ITM_MagicWandMagical : Item, IEntityTrigger
{
    ...
    public override bool Use(PlayerManager pm)
    {
        ...
        {
            Destroy(gameObject);
            CoreGameManager.Instance.audMan.PlaySingle(wandBreak);
            return true;
        }
        if (cooldown > 0f)
        {
    ...
    private IEnumerator MagicShoot(PlayerManager pm)
    {
        ...
        while (cool > 0f)
        {
            if (!CoreGameManager.Instance.audMan.QueuedAudioIsPlaying) // I ain't loopin' dat.
                CoreGameManager.Instance.audMan.QueueAudio(wandPrep, true);
        ...
        CoreGameManager.Instance.audMan.PlaySingle(wandUse);
        existCooldown = 5f;
    ...
    public void EntityTriggerEnter(Entity entity, Collider other, bool isValid)
    {
        ...
        if (flag)
        {
            ...
            if (component != null && ec.offices.Count > 0)
            {
                ...
                entity.Teleport(ec.offices[index].RandomEntitySafeCellNoGarbage().CenterWorldPosition);
